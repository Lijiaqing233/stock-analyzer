...
`/api/stocks` returns score, rating, confidence, factor scores, factor point contributions, thesis text, data-quality coverage, and risk/data flags for every stock. `/api/diagnostics` summarizes required-field coverage, fundamental coverage, provider errors, average model confidence, factor averages, and the most flagged names in the current universe.
